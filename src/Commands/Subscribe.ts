...
export default new SlashCommand()
    .setName('subscribe')
    .setDescription('Subscribes you to daily Cho gifs in your DMs.')
    .setCallback(async interaction => {
        ...
        if (user.subscribed)
            return interaction.reply({
                content: 'You are already subscribed to daily Cho gifs!',
                ephemeral: true,
            });

        await collection.updateOne(user, { $set: { subscribed: true } });

        const message = 'You are now subscribed to daily Cho gifs!\nYou will receive gifs daily at 9 AM UTC.';
